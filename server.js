...
const MATCH_SIZE=4,MATCH_AI_WAIT_MS=Math.max(1000,Number(process.env.MATCH_AI_WAIT_MS)||60000),SCORE_BY_RANK={1:60,2:30,3:-30,4:-60},ACCOUNT_SESSION_MS=30*24*60*60*1000;
let cardUid = 1;
...
function createRandomMatch(entries){const first=entries[0],created=createRoom(first.account.display_name,first.account,'random',first.profile),room=created.room,sessions=new Map([[first.account.id,{code:room.code,token:created.player.id}]]);for(const entry of entries.slice(1)){const player=makePlayer(entry.account.display_name,room.players.length,false,entry.account.id,entry.profile);room.players.push(player);sessions.set(entry.account.id,{code:room.code,token:player.id})}while(room.players.length<MATCH_SIZE){const bot=makePlayer(`AI ${room.players.filter(p=>p.isBot).length+1}`,room.players.length,true);room.players.push(bot)}room.players.forEach(p=>p.ready=true);room.hostId=null;room.settings=normalizeSettings();startGame(room);for(const entry of entries){removeFromMatchQueue(entry.account.id);clearMatchTimer(entry);const ticket=matchmakingTickets.get(entry.account.id);if(ticket){ticket.state='matched';ticket.match=sessions.get(entry.account.id);ticket.matchedAt=Date.now()}}return room}
