...
const rooms = new Map();
const memoryAccountsById=new Map(),memoryAccountsByName=new Map(),memoryAccountSessions=new Map(),memoryRankedResults=new Set();
const matchmakingTickets=new Map(),matchmakingQueue=[];
const MATCH_SIZE=4,MATCH_AI_WAIT_MS=20000,RP_BY_PLACEMENT={1:60,2:30,3:-30,4:-60},SP_BY_PLACEMENT={1:120,2:80,3:50,4:30},ACCOUNT_SESSION_MS=30*24*60*60*1000;
const RANK_TIERS=[{id:'iron',name:'아이언',min:0},{id:'bronze',name:'브론즈',min:300},{id:'silver',name:'실버',min:600},{id:'gold',name:'골드',min:900},{id:'platinum',name:'플래티넘',min:1200},{id:'diamond',name:'다이아',min:1500},{id:'master',name:'마스터',min:1800}];
const RELICS={
  painAverse:{name:'아픈거 싫어',rarity:'common',icon:'◇',text:'확률형 방어 카드의 성공 확률이 90%로 증가합니다.'},
  tank:{name:'탱거의 자질',rarity:'common',icon:'⬡',text:'공격을 받을 때 20% 확률로 피해를 완전히 방어합니다.'},
  hotHand:{name:'매운 손',rarity:'rare',icon:'♨',text:'내가 발생시키는 모든 공격 피해가 1 증가합니다.'},
  hotDefense:{name:'매운 방어',rarity:'rare',icon:'▣',text:'수치형 방어 카드의 방어량이 1 증가합니다.'},
  lastStand:{name:'최후의 기합',rarity:'rare',icon:'!',text:'치명적인 피해를 처음 한 번 버티고 HP 1로 생존합니다.'},
  return:{name:'회귀',rarity:'legendary',icon:'↺',text:'사망할 때 단 한 번 HP 10으로 부활합니다.'},
  blessing:{name:'신의 가호',rarity:'legendary',icon:'✦',text:'피해량이 2 이하인 공격을 항상 완전히 방어합니다.'}
};
const RELIC_IDS=Object.keys(RELICS);
const RELIC_DRAW_COST=160,RELIC_TEN_DRAW_COST=1600,RELIC_LEGENDARY_RATE=0.006,RELIC_RARE_RATE=0.144;
let cardUid = 1;
...
function safePoints(value){return Math.max(0,Math.trunc(Number(value)||0))}
function isSignalPointsSchemaError(error){const message=String(error?.message||error||'');return message.includes('signal_points')||message.includes('p_points')||message.includes('new_points')||message.includes('42703')||message.includes('PGRST202')}
function isRelicInventorySchemaError(error){const message=String(error?.message||error||'');return message.includes('relic_inventory')||message.includes('apply_relic_draw')||message.includes('p_results')||message.includes('PGRST202')||message.includes('42703')}
function normalizeInventory(value){const source=value&&typeof value==='object'&&!Array.isArray(value)?value:{},out={};for(const id of RELIC_IDS){const count=Math.max(0,Math.trunc(Number(source[id])||0));if(count)out[id]=count}return out}
function rankInfo(value){const rp=safeScore(value),tier=[...RANK_TIERS].reverse().find(x=>rp>=x.min)||RANK_TIERS[0],index=RANK_TIERS.indexOf(tier),next=RANK_TIERS[index+1]||null,local=rp-tier.min,division=next?['III','II','I'][Math.min(2,Math.floor(local/100))]:null,divisionStart=tier.min+(division?Math.floor(local/100)*100:0),divisionEnd=next?Math.min(next.min,divisionStart+100):null;return {id:tier.id,name:tier.name,division,label:division?`${tier.name} ${division}`:tier.name,rp,nextRp:divisionEnd,progress:divisionEnd?Math.max(0,Math.min(100,Math.round((rp-divisionStart)/(divisionEnd-divisionStart)*100))):100}}
...
function publicAccount(account){return account?{id:account.id,username:account.username,displayName:account.display_name,rp:safeScore(account.score),rank:rankInfo(account.score),points:safePoints(account.signal_points),relicInventory:normalizeInventory(account.relic_inventory)}:null}
async function findPersistedAccount(base){try{const rows=await databaseRequest(`${base}&select=id,username,display_name,password_salt,password_hash,score,signal_points,relic_inventory&limit=1`);return rows?.[0]||null}catch(error){if(!isRelicInventorySchemaError(error)&&!isSignalPointsSchemaError(error))throw error;try{const rows=await databaseRequest(`${base}&select=id,username,display_name,password_salt,password_hash,score,signal_points&limit=1`);return rows?.[0]?{...rows[0],relic_inventory:{}}:null}catch(pointsError){if(!isSignalPointsSchemaError(pointsError))throw pointsError;const rows=await databaseRequest(`${base}&select=id,username,display_name,password_salt,password_hash,score&limit=1`);return rows?.[0]?{...rows[0],signal_points:0,relic_inventory:{}}:null}}}
async function findAccountByUsername(username){if(!PERSISTENCE_ENABLED)return memoryAccountsByName.get(username)||null;return findPersistedAccount(`player_accounts?username=eq.${encodeURIComponent(username)}`)}
async function findAccountById(id){if(!PERSISTENCE_ENABLED)return memoryAccountsById.get(id)||null;return findPersistedAccount(`player_accounts?id=eq.${encodeURIComponent(id)}`)}
async function createAccount(username,password,displayName){username=normalizeUsername(username);validateCredentials(username,password);displayName=String(displayName||username).trim().slice(0,10);if(!displayName)throw Error('닉네임을 입력하세요.');if(await findAccountByUsername(username))throw Error('이미 사용 중인 아이디입니다.');const salt=crypto.randomBytes(16).toString('hex'),account={id:crypto.randomUUID(),username,display_name:displayName,password_salt:salt,password_hash:passwordHash(password,salt),score:0,signal_points:0,relic_inventory:{}};if(PERSISTENCE_ENABLED){try{const rows=await databaseRequest('player_accounts',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(account)});return rows[0]}catch(error){if(!isRelicInventorySchemaError(error)&&!isSignalPointsSchemaError(error))throw error;const fallback={...account};delete fallback.relic_inventory;if(isSignalPointsSchemaError(error))delete fallback.signal_points;const rows=await databaseRequest('player_accounts',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(fallback)});return {...rows[0],signal_points:rows[0]?.signal_points||0,relic_inventory:{}}}}memoryAccountsById.set(account.id,account);memoryAccountsByName.set(username,account);return account}
async function issueAccountSession(account){const raw=token()+token(),hash=crypto.createHash('sha256').update(raw).digest('hex'),expiresAt=Date.now()+ACCOUNT_SESSION_MS;if(PERSISTENCE_ENABLED)await databaseRequest('account_sessions',{method:'POST',body:JSON.stringify({token_hash:hash,account_id:account.id,expires_at:new Date(expiresAt).toISOString()})});else memoryAccountSessions.set(hash,{accountId:account.id,expiresAt});return raw}
...
async function applyRankedReward(roomCode,accountId,rank,rpDelta,spReward){const key=`${roomCode}:${accountId}`;if(PERSISTENCE_ENABLED){try{const rows=await databaseRequest('rpc/apply_ranked_result',{method:'POST',body:JSON.stringify({p_room_code:roomCode,p_account_id:accountId,p_placement:rank,p_delta:rpDelta,p_points:spReward})}),row=rows?.[0]||{};return {rp:safeScore(row.new_score),points:safePoints(row.new_points)}}catch(error){if(!isSignalPointsSchemaError(error))throw error;const rows=await databaseRequest('rpc/apply_ranked_result',{method:'POST',body:JSON.stringify({p_room_code:roomCode,p_account_id:accountId,p_placement:rank,p_delta:rpDelta})}),row=rows?.[0]||{};return {rp:safeScore(row.new_score),points:0}}}if(memoryRankedResults.has(key)){const old=await findAccountById(accountId);return {rp:safeScore(old?.score),points:safePoints(old?.signal_points)}}memoryRankedResults.add(key);const account=await findAccountById(accountId);if(!account)return {rp:0,points:0};account.score=safeScore(safeScore(account.score)+rpDelta);account.signal_points=safePoints(account.signal_points)+safePoints(spReward);return {rp:account.score,points:account.signal_points}}
function drawRelicId(){const roll=crypto.randomInt(0,1000000)/1000000,rarity=roll<RELIC_LEGENDARY_RATE?'legendary':roll<RELIC_LEGENDARY_RATE+RELIC_RARE_RATE?'rare':'common',pool=RELIC_IDS.filter(id=>RELICS[id].rarity===rarity);return pool[crypto.randomInt(0,pool.length)]}
function equippedRelicForAccount(account,relicId){const id=normalizeRelic(relicId)?.id;if(!account||!id)return null;return normalizeInventory(account.relic_inventory)[id]>0?id:null}
async function drawAccountRelics(account,count){count=Number(count)===10?10:1;const cost=count===10?RELIC_TEN_DRAW_COST:RELIC_DRAW_COST;if(safePoints(account.signal_points)<cost)throw Error(`SP가 부족합니다. ${count===10?'10연속':'1회'} 뽑기에는 ${cost} SP가 필요합니다.`);const ids=Array.from({length:count},drawRelicId),gained={};for(const id of ids)gained[id]=(gained[id]||0)+1;let points,inventory;if(PERSISTENCE_ENABLED){try{const rows=await databaseRequest('rpc/apply_relic_draw',{method:'POST',body:JSON.stringify({p_account_id:account.id,p_cost:cost,p_results:gained})}),row=rows?.[0];if(!row)throw Error('SP가 부족하거나 계정 정보를 찾을 수 없습니다.');points=safePoints(row.new_points);inventory=normalizeInventory(row.new_inventory)}catch(error){if(isRelicInventorySchemaError(error))throw Error('유물 뽑기 데이터베이스 설정이 필요합니다. 최신 supabase-setup.sql을 실행하세요.');throw error}}else{account.signal_points=safePoints(account.signal_points)-cost;inventory=normalizeInventory(account.relic_inventory);for(const [id,amount] of Object.entries(gained))inventory[id]=(inventory[id]||0)+amount;account.relic_inventory=inventory;points=account.signal_points}const refreshed=PERSISTENCE_ENABLED?await findAccountById(account.id):account;return {results:ids.map(id=>({id,...RELICS[id]})),account:publicAccount(refreshed||{...account,signal_points:points,relic_inventory:inventory})}}
async function getLeaderboard(viewerId=null){
  let accounts;
  if(PERSISTENCE_ENABLED){try{accounts=await databaseRequest('player_accounts?select=id,display_name,score,signal_points,created_at&order=score.desc,created_at.asc&limit=100')}catch(error){if(!isSignalPointsSchemaError(error))throw error;accounts=(await databaseRequest('player_accounts?select=id,display_name,score,created_at&order=score.desc,created_at.asc&limit=100')||[]).map(entry=>({...entry,signal_points:0}))}}
  else accounts=[...memoryAccountsById.values()].sort((a,b)=>(Number(b.score)||0)-(Number(a.score)||0)||String(a.id).localeCompare(String(b.id))).slice(0,100);
  ...
  return (accounts||[]).map((entry,index)=>{
    ...
    previousScore=score;
    return {position:rank,displayName:String(entry.display_name||'플레이어').slice(0,10),rp:score,tier:rankInfo(score),points:safePoints(entry.signal_points),isMe:entry.id===viewerId};
  });
}
...
async function loadPersistedRoom(code){
  ...
  if(Number(room.rulesVersion||0)<11)room.rulesVersion=11;room.matchType=room.matchType==='random'?'random':'custom';room.ranked=room.matchType==='random';room.ratingState=room.ratingState||null;room.ratingResults=Array.isArray(room.ratingResults)?room.ratingResults:[];for(const p of room.players)p.accountId=p.accountId||null;
...
function normalizeRelic(value){const id=typeof value==='string'?value:value?.id;return RELICS[id]?{id,...RELICS[id]}:null}
function publicRelic(value){const relic=normalizeRelic(value);return relic?{...relic}:null}
...
const PROFILE_AVATARS=new Set(['signal','phantom','viper','nova','rook','ace']);
const PROFILE_FRAMES=new Set(['carbon','neon','gold','crimson','void']);
const PROFILE_TITLES=new Set(['신호 추적자','생존 전문가','심리전의 달인','전장의 지휘관','최후의 생존자','전술 AI']);
function normalizeProfile(value={}){const avatar=PROFILE_AVATARS.has(value?.avatar)?value.avatar:'signal',frame=PROFILE_FRAMES.has(value?.frame)?value.frame:'carbon',title=PROFILE_TITLES.has(value?.title)?value.title:'신호 추적자',image=typeof value?.image==='string'&&/^data:image\/(?:png|jpeg|webp);base64,[a-zA-Z0-9+/=]+$/.test(value.image)&&value.image.length<=180000?value.image:'';return {avatar,frame,title,image}}
function makePlayer(name,index,isBot=false,accountId=null,profile={},rankRp=0,relicId=null){return {id:token(),accountId:accountId||null,name:String(name||'플레이어').trim().slice(0,10),profile:normalizeProfile(isBot?{avatar:['rook','viper','phantom'][index%3],frame:'void',title:'전술 AI'}:profile),rankRp:safeScore(rankRp),relic:normalizeRelic(relicId||(isBot?pick(RELIC_IDS):null)),relicState:{lastStandUsed:false,revived:false},seat:index,isBot:!!isBot,ready:!!isBot,connected:true,hp:20,alive:true,hand:[],discard:[],items:[],itemUsedThisTurn:false,turnsTaken:0,choice:null,choiceUsed:false,buffs:blankBuffs(),intel:[]}}
...
function migratePlayerItems(room,p){
  p.isBot=!!p.isBot;if(p.isBot){p.ready=true;p.connected=true}p.profile=normalizeProfile(p.profile);p.buffs={...blankBuffs(),...(p.buffs||{})};p.items=Array.isArray(p.items)?p.items:[];p.itemUsedThisTurn=!!p.itemUsedThisTurn;p.intel=Array.isArray(p.intel)?p.intel:[];
...
function activateRelic(room,p,text){const relic=normalizeRelic(p?.relic);if(!relic)return;addEffect(room,'relic',p,p,relic,text||relic.text);addLog(room,`${p.name}의 유물 발동 · ${relic.name} · ${text||relic.text}`,'good')}
...
function hurt(room,p,n,source){if(!p.alive||n<=0)return;const lethal=n>=p.hp,relic=normalizeRelic(p.relic),state=p.relicState||(p.relicState={lastStandUsed:false,revived:false});if(lethal&&relic?.id==='lastStand'&&!state.lastStandUsed){state.lastStandUsed=true;p.hp=1;activateRelic(room,p,'치명적인 피해를 버티고 HP 1로 생존');return}if(lethal&&relic?.id==='return'&&!state.revived){state.revived=true;p.hp=10;p.alive=true;p.buffs.doomTurns=0;p.buffs.doomFresh=false;p.buffs.doomSourceId=null;activateRelic(room,p,'사망을 되돌리고 HP 10으로 부활');return}p.hp=Math.max(0,p.hp-n);addLog(room,`${p.name} ${n} 피해 · ${source}`,'hit');if(p.hp===0){p.alive=false;p.choice=null;p.buffs.doomTurns=0;p.buffs.doomFresh=false;p.buffs.doomSourceId=null;delete room.pendingAttacks[p.id];room.eliminationOrder=room.eliminationOrder||[];if(!room.eliminationOrder.includes(p.id))room.eliminationOrder.push(p.id);addEffect(room,'elimination',p,p,null,`${p.name} 탈락`);addLog(room,`${p.name} 탈락`,'death')}}
function consume(p,c){const i=p.hand.findIndex(x=>x.uid===c.uid);if(i>=0)p.discard.push(p.hand.splice(i,1)[0]);p.choiceUsed=true}
function publicCard(c){return c?{id:c.id,uid:c.uid,name:c.name,type:c.type,rarity:c.rarity,icon:c.icon||null,text:c.text,chooseTarget:!!c.chooseTarget}:null}
...
function removeFromMatchQueue(accountId){for(let i=matchmakingQueue.length-1;i>=0;i--)if(matchmakingQueue[i]===accountId)matchmakingQueue.splice(i,1)}
function clearMatchTimer(ticket){if(ticket?.autoFillTimer){clearTimeout(ticket.autoFillTimer);ticket.autoFillTimer=null}}
function matchTicketView(ticket){if(!ticket)return {state:'idle'};const elapsed=Math.max(0,Date.now()-ticket.joinedAt);if(ticket.state==='queued'&&elapsed>=MATCH_AI_WAIT_MS)autoFillRandomMatch(ticket.account.id);const currentElapsed=Math.max(0,Date.now()-ticket.joinedAt);return {state:ticket.state,joinedAt:ticket.joinedAt,autoFillAt:ticket.joinedAt+MATCH_AI_WAIT_MS,elapsedSeconds:Math.floor(currentElapsed/1000),aiAllowed:ticket.state==='queued'&&currentElapsed>=MATCH_AI_WAIT_MS,waitingCount:matchmakingQueue.length,match:ticket.match||null}}
function createRandomMatch(entries){const first=entries[0],created=createRoom(first.account.display_name,first.account,'random',first.profile,first.relicId),room=created.room,sessions=new Map([[first.account.id,{code:room.code,token:created.player.id}]]);for(const entry of entries.slice(1)){const player=makePlayer(entry.account.display_name,room.players.length,false,entry.account.id,entry.profile,entry.account.score,entry.relicId);room.players.push(player);sessions.set(entry.account.id,{code:room.code,token:player.id})}const averageRp=Math.round(entries.reduce((sum,x)=>sum+safeScore(x.account.score),0)/Math.max(1,entries.length));while(room.players.length<MATCH_SIZE){const bot=makePlayer(`AI ${room.players.filter(p=>p.isBot).length+1}`,room.players.length,true,null,{},averageRp);room.players.push(bot)}room.players.forEach(p=>p.ready=true);room.hostId=null;room.settings=normalizeSettings();startGame(room);for(const entry of entries){removeFromMatchQueue(entry.account.id);clearMatchTimer(entry);const ticket=matchmakingTickets.get(entry.account.id);if(ticket){ticket.state='matched';ticket.match=sessions.get(entry.account.id);ticket.matchedAt=Date.now()}}return room}
...
async function joinMatchmaking(account,profile={},relicId=null){let ticket=matchmakingTickets.get(account.id);if(ticket?.state==='matched'){const room=await getRoom(ticket.match?.code),player=room?.players.find(p=>p.accountId===account.id);if(room&&room.status!=='finished'&&room.status!=='deleted'&&player?.alive)return matchTicketView(ticket);clearMatchTimer(ticket);matchmakingTickets.delete(account.id);ticket=null}if(ticket?.state==='queued')return matchTicketView(ticket);ticket={account,profile:normalizeProfile(profile),relicId:normalizeRelic(relicId)?.id||null,state:'queued',joinedAt:Date.now(),match:null,autoFillTimer:null};ticket.autoFillTimer=setTimeout(()=>{try{autoFillRandomMatch(account.id)}catch(error){console.error(`[matchmaking] auto fill ${account.id}:`,error.message)}},MATCH_AI_WAIT_MS+50);ticket.autoFillTimer.unref?.();matchmakingTickets.set(account.id,ticket);matchmakingQueue.push(account.id);await tryFullRandomMatch();return matchTicketView(ticket)}
function cancelMatchmaking(account){const ticket=matchmakingTickets.get(account.id);if(ticket?.state==='queued'){clearMatchTimer(ticket);removeFromMatchQueue(account.id);matchmakingTickets.delete(account.id)}return {state:'idle'}}
function autoFillRandomMatch(accountId){const requester=matchmakingTickets.get(accountId);if(!requester||requester.state!=='queued'||Date.now()-requester.joinedAt<MATCH_AI_WAIT_MS)return requester?matchTicketView(requester):{state:'idle'};const ids=[accountId,...matchmakingQueue.filter(id=>id!==accountId)].slice(0,MATCH_SIZE),entries=ids.map(id=>matchmakingTickets.get(id)).filter(x=>x?.state==='queued');if(!entries.length)return {state:'idle'};createRandomMatch(entries);return matchTicketView(requester)}
function fillRandomMatchWithAi(account){const requester=matchmakingTickets.get(account.id);if(!requester||requester.state!=='queued')throw Error('랜덤 매칭 대기 중이 아닙니다.');if(Date.now()-requester.joinedAt<MATCH_AI_WAIT_MS)throw Error(`AI 자동 충원까지 ${Math.ceil(MATCH_AI_WAIT_MS/1000)}초를 기다려 주세요.`);return autoFillRandomMatch(account.id)}
...
function view(room,me){
  ...
  return {
    code:room.code,matchType:room.matchType||'custom',ranked:!!room.ranked,status:room.status,phase:room.phase,turnNumber:room.turnNumber||0,direction:room.direction===-1?-1:1,deadline:room.deadline,event:room.event,winner:room.winner,winners:room.winners||[],endReason:room.endReason||null,ranking:(room.ranking||[]).map(row=>{const rating=(room.ratingResults||[]).find(x=>x.playerId===row.id);return {...row,rpDelta:rating?.rpDelta??null,newRp:rating?.newRp??null,spReward:rating?.spReward??null,newPoints:rating?.newPoints??null}}),settings:normalizeSettings(room.settings),roundState:room.roundState||null,revealIndex:room.revealIndex,revealCurrentId:room.revealCurrentId,currentPlayerId:room.currentPlayerId,currentIncoming:room.currentPlayerId&&room.pendingAttacks?.[room.currentPlayerId]?room.pendingAttacks[room.currentPlayerId]:null,
    hostId:room.hostId,meId:me.id,startPlayerId:room.players[room.startIndex]?.id,
    players:room.players.map(p=>({id:p.id,name:p.name,profile:normalizeProfile(p.profile),rank:rankInfo(p.rankRp),relic:publicRelic(p.relic),relicState:p.relicState||{},seat:p.seat,isBot:!!p.isBot,ready:p.ready,connected:p.connected,hp:p.hp,alive:p.alive,handCount:p.hand.length,itemCount:p.items?.length||0,legendaryCount:p.hand.filter(c=>c.rarity==='legendary').length,turnsTaken:p.turnsTaken||0,pendingAttack:!!room.pendingAttacks?.[p.id],buffs:p.buffs,submitted:!!p.choice,choice:p.choice?(visibleChoice(p)?{kind:p.choice.kind,card:publicCard(p.choice.card)}:{kind:'hidden'}):null})),
...
function beginContinuousTurn(room,p,initial=false){
  ...
  if(room.pendingAttacks[p.id]&&relicBlocksIncoming(room,p,room.pendingAttacks[p.id])){const blocked=room.pendingAttacks[p.id];delete room.pendingAttacks[p.id];addLog(room,`${p.name}의 ${p.relic.name} · ${blocked.name} 무효`,'good')}
  ...
  if(p.buffs.doomTurns>0&&doomIncoming&&doomAttacker?.alive){
    ...
    if(!p.alive){if(checkWinner(room))return;if(completeRoundTurn(room,p)==='finished')return;const next=nextAlive(room,p),delay=1800;room.phase='turn_result';room.deadline=Date.now()+delay;broadcast(room);room.timer=setTimeout(()=>beginContinuousTurn(room,next),delay);return}
  }
  ...
  while(p.hand.length>12)p.discard.push(p.hand.pop());const turnDelay=p.isBot?crypto.randomInt(2600,3801):normalizeSettings(room.settings).turnSeconds*1000;room.deadline=Date.now()+turnDelay;const incoming=room.pendingAttacks[p.id];
...
function relicAttackBonus(room,p){if(p.relic?.id!=='hotHand')return 0;activateRelic(room,p,'이번 공격 피해 +1');return 1}
...
function resolveSubmittedTurn(room,p){
  ...
  if(checkWinner(room))return;const freshEffects=room.effects.filter(effect=>effect.seq>effectsBefore),added=Math.max(1,freshEffects.length),hasRoulette=freshEffects.some(effect=>effect.type==='roulette'),hasRelic=freshEffects.some(effect=>effect.type==='relic'),delay=Math.max(hasRoulette?added*1100+1900:1250,hasRelic?added*720+1700:added*720+450);room.deadline=Date.now()+delay;broadcast(room);room.timer=setTimeout(()=>advanceTurn(room),delay);
}
function resolveIncomingTurn(room,p,choice,incoming,attacker){
  ...
  if(relicBlocksIncoming(room,p,incoming)){addLog(room,`${p.name}의 ${p.relic.name} · ${incoming.name} 무효`,'good');resolveFreeTurn(room,p,choice);return}
...
function beginRankedSettlement(room){if(room.matchType!=='random'||room.ratingState==='complete'||room.ratingInFlight)return;if(!room.ranking?.length)buildRanking(room);if(!room.ratingResults?.length)room.ratingResults=room.ranking.map(row=>{const player=room.players.find(p=>p.id===row.id);return player?.accountId?{playerId:player.id,accountId:player.accountId,rank:row.rank,rpDelta:RP_BY_PLACEMENT[row.rank]||0,spReward:SP_BY_PLACEMENT[row.rank]||0,newRp:null,newPoints:null}:null}).filter(Boolean);room.ratingState='pending';room.ratingInFlight=true;Promise.all(room.ratingResults.map(async result=>{const reward=await applyRankedReward(room.code,result.accountId,result.rank,result.rpDelta,result.spReward);result.newRp=reward.rp;result.newPoints=reward.points;const player=room.players.find(p=>p.id===result.playerId);if(player)player.rankRp=reward.rp;return result})).then(()=>{room.ratingState='complete';room.ratingInFlight=false;addLog(room,'랭크 RP 및 SIGNAL POINT 지급 완료','system');broadcast(room)}).catch(error=>{room.ratingInFlight=false;console.error(`[rating] ${room.code}:`,error.message);persistRoom(room)})}
...
async function api(req,res,url){
  try{
    ...
    if(req.method==='POST'&&url.pathname==='/api/account/signup'){const b=await readBody(req),account=await createAccount(b.username,b.password,b.displayName),authToken=await issueAccountSession(account);return json(res,200,{authToken,account:publicAccount(account)})}
    ...
    if(req.method==='POST'&&url.pathname==='/api/relic/draw'){const b=await readBody(req),account=await accountFromToken(b.authToken);return json(res,200,await drawAccountRelics(account,b.count))}
    ...
    if(req.method==='POST'&&url.pathname==='/api/match/join'){const b=await readBody(req),account=await accountFromToken(b.authToken);return json(res,200,await joinMatchmaking(account,b.profile,equippedRelicForAccount(account,b.relicId)))}
    ...
    if(req.method==='POST'&&url.pathname==='/api/match/fill-ai'){const b=await readBody(req),account=await accountFromToken(b.authToken);return json(res,200,fillRandomMatchWithAi(account))}
    if(req.method==='POST'&&url.pathname==='/api/room/create'){const b=await readBody(req),account=await accountFromToken(b.authToken,false),x=createRoom(account?.display_name||b.name,account,'custom',b.profile,equippedRelicForAccount(account,b.relicId));return json(res,200,{code:x.room.code,token:x.player.id})}
    if(req.method==='POST'&&url.pathname==='/api/room/join'){const b=await readBody(req),account=await accountFromToken(b.authToken,false),x=await joinRoom(String(b.code||''),account?.display_name||b.name,account,b.profile,equippedRelicForAccount(account,b.relicId));return json(res,200,{code:x.room.code,token:x.player.id})}
    ...
    if(req.method==='POST'&&url.pathname==='/api/room/delete'){const b=await readBody(req),{room,player}=await auth(b);if(room.matchType==='random')throw Error('랜덤 매치는 삭제할 수 없습니다.');if(room.hostId!==player.id)throw Error('방장만 방을 삭제할 수 있습니다.');deleteRoom(room);return json(res,200,{ok:true})}
...
function staticFile(req,res,url){
  ...
  const ext=path.extname(file);const types={'.html':'text/html; charset=utf-8','.js':'application/javascript; charset=utf-8','.css':'text/css; charset=utf-8','.png':'image/png','.jpg':'image/jpeg','.jpeg':'image/jpeg','.webp':'image/webp','.svg':'image/svg+xml','.mp4':'video/mp4','.mp3':'audio/mpeg','.m4a':'audio/mp4','.wav':'audio/wav'},contentType=types[ext]||'application/octet-stream',cacheControl=ext==='.html'?'no-store':'public, max-age=3600',media=['.mp4','.mp3','.m4a','.wav'].includes(ext),size=fs.statSync(file).size;
  if(media&&req.headers.range){const match=/bytes=(\d*)-(\d*)/.exec(req.headers.range);if(match){const start=match[1]?Number(match[1]):0,end=Math.min(match[2]?Number(match[2]):size-1,size-1);if(Number.isFinite(start)&&Number.isFinite(end)&&start<=end&&start<size){res.writeHead(206,{'content-type':contentType,'content-length':end-start+1,'content-range':`bytes ${start}-${end}/${size}`,'accept-ranges':'bytes','cache-control':cacheControl});fs.createReadStream(file,{start,end}).pipe(res);return}}}
  res.writeHead(200,{'content-type':contentType,'content-length':size,...(media?{'accept-ranges':'bytes'}:{}),'cache-control':cacheControl});fs.createReadStream(file).pipe(res);
}
