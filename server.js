...
async function findAccountByUsername(username){if(!PERSISTENCE_ENABLED)return memoryAccountsByName.get(username)||null;const rows=await databaseRequest(`player_accounts?username=eq.${encodeURIComponent(username)}&select=id,username,display_name,password_salt,password_hash,score,signal_points&limit=1`);return rows?.[0]||null}
async function findAccountById(id){if(!PERSISTENCE_ENABLED)return memoryAccountsById.get(id)||null;const rows=await databaseRequest(`player_accounts?id=eq.${encodeURIComponent(id)}&select=id,username,display_name,password_salt,password_hash,score,signal_points&limit=1`);return rows?.[0]||null}
...
async function applyRankedReward(roomCode,accountId,rank,rpDelta,spReward){const key=`${roomCode}:${accountId}`;if(PERSISTENCE_ENABLED){const rows=await databaseRequest('rpc/apply_ranked_result',{method:'POST',body:JSON.stringify({p_room_code:roomCode,p_account_id:accountId,p_placement:rank,p_delta:rpDelta,p_points:spReward})}),row=rows?.[0]||{};return {rp:safeScore(row.new_score),points:safePoints(row.new_points)}}if(memoryRankedResults.has(key)){const old=await findAccountById(accountId);return {rp:safeScore(old?.score),points:safePoints(old?.signal_points)}}memoryRankedResults.add(key);const account=await findAccountById(accountId);if(!account)return {rp:0,points:0};account.score=safeScore(safeScore(account.score)+rpDelta);account.signal_points=safePoints(account.signal_points)+safePoints(spReward);return {rp:account.score,points:account.signal_points}}
async function getLeaderboard(viewerId=null){
  const accounts=PERSISTENCE_ENABLED
    ? await databaseRequest('player_accounts?select=id,display_name,score,signal_points,created_at&order=score.desc,created_at.asc&limit=100')
