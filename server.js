...
function publicAccount(account){return account?{id:account.id,username:account.username,displayName:account.display_name,rp:safeScore(account.score),rank:rankInfo(account.score),points:safePoints(account.signal_points)}:null}
async function findAccountByUsername(username){if(!PERSISTENCE_ENABLED)return memoryAccountsByName.get(username)||null;const base=`player_accounts?username=eq.${encodeURIComponent(username)}`;try{const rows=await databaseRequest(`${base}&select=id,username,display_name,password_salt,password_hash,score,signal_points&limit=1`);return rows?.[0]||null}catch(error){if(!isSignalPointsSchemaError(error))throw error;const rows=await databaseRequest(`${base}&select=id,username,display_name,password_salt,password_hash,score&limit=1`);return rows?.[0]?{...rows[0],signal_points:0}:null}}
async function findAccountById(id){if(!PERSISTENCE_ENABLED)return memoryAccountsById.get(id)||null;const base=`player_accounts?id=eq.${encodeURIComponent(id)}`;try{const rows=await databaseRequest(`${base}&select=id,username,display_name,password_salt,password_hash,score,signal_points&limit=1`);return rows?.[0]||null}catch(error){if(!isSignalPointsSchemaError(error))throw error;const rows=await databaseRequest(`${base}&select=id,username,display_name,password_salt,password_hash,score&limit=1`);return rows?.[0]?{...rows[0],signal_points:0}:null}}
async function createAccount(username,password,displayName){username=normalizeUsername(username);validateCredentials(username,password);displayName=String(displayName||username).trim().slice(0,10);if(!displayName)throw Error('닉네임을 입력하세요.');if(await findAccountByUsername(username))throw Error('이미 사용 중인 아이디입니다.');const salt=crypto.randomBytes(16).toString('hex'),account={id:crypto.randomUUID(),username,display_name:displayName,password_salt:salt,password_hash:passwordHash(password,salt),score:0,signal_points:0};if(PERSISTENCE_ENABLED){try{const rows=await databaseRequest('player_accounts',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(account)});return rows[0]}catch(error){if(!isSignalPointsSchemaError(error))throw error;const {signal_points,...legacyAccount}=account,rows=await databaseRequest('player_accounts',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify(legacyAccount)});return {...rows[0],signal_points:0}}}memoryAccountsById.set(account.id,account);memoryAccountsByName.set(username,account);return account}
...
async function api(req,res,url){
  try{
    ...
    if(req.method==='POST'&&url.pathname==='/api/match/join'){const b=await readBody(req),account=await accountFromToken(b.authToken);return json(res,200,await joinMatchmaking(account,b.profile,b.relicId))}
    ...
    if(req.method==='POST'&&url.pathname==='/api/room/create'){const b=await readBody(req),account=await accountFromToken(b.authToken,false),x=createRoom(account?.display_name||b.name,account,'custom',b.profile,b.relicId);return json(res,200,{code:x.room.code,token:x.player.id})}
    if(req.method==='POST'&&url.pathname==='/api/room/join'){const b=await readBody(req),account=await accountFromToken(b.authToken,false),x=await joinRoom(String(b.code||''),account?.display_name||b.name,account,b.profile,b.relicId);return json(res,200,{code:x.room.code,token:x.player.id})}
