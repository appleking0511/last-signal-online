...
function createRandomMatch(entries){const first=entries[0],created=createRoom(first.account.display_name,first.account,'random',first.profile),room=created.room,sessions=new Map([[first.account.id,{code:room.code,token:created.player.id}]]);for(const entry of entries.slice(1)){const player=makePlayer(entry.account.display_name,room.players.length,false,entry.account.id,entry.profile,entry.account.score);room.players.push(player);sessions.set(entry.account.id,{code:room.code,token:player.id})}const averageRp=Math.round(entries.reduce((sum,x)=>sum+safeScore(x.account.score),0)/Math.max(1,entries.length));while(room.players.length<MATCH_SIZE){const bot=makePlayer(`AI ${room.players.filter(p=>p.isBot).length+1}`,room.players.length,true,null,{},averageRp);room.players.push(bot)}room.players.forEach(p=>p.ready=true);room.hostId=null;room.settings=normalizeSettings();startGame(room);for(const entry of entries){removeFromMatchQueue(entry.account.id);clearMatchTimer(entry);const ticket=matchmakingTickets.get(entry.account.id);if(ticket){ticket.state='matched';ticket.match=sessions.get(entry.account.id);ticket.matchedAt=Date.now()}}return room}
...
async function joinMatchmaking(account,profile={}){let ticket=matchmakingTickets.get(account.id);if(ticket?.state==='matched'){const room=await getRoom(ticket.match?.code),player=room?.players.find(p=>p.accountId===account.id);if(room&&room.status!=='finished'&&room.status!=='deleted'&&player?.alive)return matchTicketView(ticket);clearMatchTimer(ticket);matchmakingTickets.delete(account.id);ticket=null}if(ticket?.state==='queued')return matchTicketView(ticket);ticket={account,profile:normalizeProfile(profile),state:'queued',joinedAt:Date.now(),match:null,autoFillTimer:null};ticket.autoFillTimer=setTimeout(()=>{try{autoFillRandomMatch(account.id)}catch(error){console.error(`[matchmaking] auto fill ${account.id}:`,error.message)}},MATCH_AI_WAIT_MS+50);ticket.autoFillTimer.unref?.();matchmakingTickets.set(account.id,ticket);matchmakingQueue.push(account.id);await tryFullRandomMatch();return matchTicketView(ticket)}
...
async function api(req,res,url){
  try{
    ...
    if(req.method==='POST'&&url.pathname==='/api/match/join'){const b=await readBody(req),account=await accountFromToken(b.authToken);return json(res,200,await joinMatchmaking(account,b.profile))}
    ...
    if(req.method==='POST'&&url.pathname==='/api/room/create'){const b=await readBody(req),account=await accountFromToken(b.authToken,false),x=createRoom(account?.display_name||b.name,account,'custom',b.profile);return json(res,200,{code:x.room.code,token:x.player.id})}
    if(req.method==='POST'&&url.pathname==='/api/room/join'){const b=await readBody(req),account=await accountFromToken(b.authToken,false),x=await joinRoom(String(b.code||''),account?.display_name||b.name,account,b.profile);return json(res,200,{code:x.room.code,token:x.player.id})}
