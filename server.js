...
const MATCH_SIZE=4,MATCH_AI_WAIT_MS=Math.max(1000,Number(process.env.MATCH_AI_WAIT_MS)||60000),RP_BY_PLACEMENT={1:60,2:30,3:-30,4:-60},SP_BY_PLACEMENT={1:120,2:80,3:50,4:30},ACCOUNT_SESSION_MS=30*24*60*60*1000;
...
function fillRandomMatchWithAi(account){const requester=matchmakingTickets.get(account.id);if(!requester||requester.state!=='queued')throw Error('랜덤 매칭 대기 중이 아닙니다.');if(Date.now()-requester.joinedAt<MATCH_AI_WAIT_MS)throw Error('AI 자동 충원까지 60초를 기다려 주세요.');return autoFillRandomMatch(account.id)}
